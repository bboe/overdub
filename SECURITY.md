...
| udp/5353, the mDNS responder | nothing, by design: discovery answers anyone who asks, and the stock firewall already accepts the port. What it can spend is bounded instead. A reply is at most one multicast a second, so the flooding a forty-byte query could otherwise buy is capped, and the answer it draws is 328 to 700 bytes; a query carrying its own answer draws no reply for the service that answer names, and none at all when it names them all; and no reply is ever logged, so it is not a way to write to `/data`. The records name the device, its address and the API port, which anything on the segment could learn by connecting anyway |
