...
func TestAFlagReadsBackWhatWasWritten(t *testing.T) {
	...
	// The name has to be persist.-prefixed, or Android does not keep it across a
	// reboot and the switch silently comes back on.
...
func TestAFlagTellsAFailedReadFromAnUnsetOne(t *testing.T) {
	// The caller picks a different default for each, so they must not look alike:
	// an unset flag is a fresh install, a failed read is a flag we cannot honour.
	oldRead := readProp
