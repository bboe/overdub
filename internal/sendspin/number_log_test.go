...
func TestAPeerSuppliedNumberCannotStretchALogLine(t *testing.T) {
	// encoding/json puts the offending literal into UnmarshalTypeError, so a peer's
	// bytes reach the log inside an error rather than inside a string the call site
	// knows to cut.
	var out lockedLog
	was := log.Writer()
	log.SetOutput(&out)
