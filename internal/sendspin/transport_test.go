...
func TestReadEnvelopeRefusesANonJSONType(t *testing.T) {
	// The body is valid JSON, so a reader that ignores the type byte parses it
	// happily: asserting errTransport alone cannot tell the two apart.
	session, server, peer := pairedSession(t)
	go func() {
...
func TestTheFrameLayoutIsFixedAgainstLiterals(t *testing.T) {
	// Every one of these is a number a peer also has to agree on, and every test
	// that builds a frame builds it from these same constants -- so nothing else in
	// this package can notice one of them moving.
	for _, c := range []struct {
		name      string
		got, want int
	}{
...
func TestAFragmentedMessageLeavesNoStateBehind(t *testing.T) {
	// Nothing else sends a second message after a fragmented one, so a reassembler
	// that forgets to clear itself looks perfectly healthy.
	session, server, peer := pairedSession(t)
