...
// The spec's worked example: the same Sentinel connection to a client with
// unpaired access disabled answers pairing_required for one activation and
// unauthorized for the other.
...
func TestPersistedRolesBecomeEmptyWhenTheConnectionStopsBeingPlaybackCapable(t *testing.T) {
	// A Sentinel connection with unpaired access off can never carry playback, so
	// roles held from before and not restated are dropped. A pairing activity would
	// abort on the method before it reached the line this is about, and a long-term
	// connection stays playback-capable even when it declares no activity.
...
func TestPairingMethodMustMatchTheMatchedPSK(t *testing.T) {
	// Supplies its own offered set, so this tests the rule rather than whatever
	// this client happens to advertise today.
...
func TestActivateTreatsAFirstMessageWithNoRolesAsEmpty(t *testing.T) {
	...
	if !s.activated {
		t.Error("session did not record that it has been activated")
	}
}
...
func TestClientHelloIsExactlyThisOnTheWire(t *testing.T) {
	// A literal for the same reason client/init has one: every other assertion in
	// this file reads the fields back through the structs that wrote them, so a
	// renamed JSON tag changes what Music Assistant sees and breaks nothing here.
