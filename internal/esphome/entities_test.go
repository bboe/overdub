...
func TestTheSendspinSwitchIsListedAndCarriesItsOwnKey(t *testing.T) {
	// The switch is the only way an operator turns the Sendspin surface off, and
	// nothing else in the tree notices if it stops being listed or if its command
	// lands on another entity: both would ship as a switch that is simply missing, or
	// one that moves the microphone instead.
	s := NewServer("kitchen", "Echo Dot", "", "00:00:5E:00:53:2A", nil)
...
func TestASendspinSwitchCommandReachesTheSwitchAndNothingElse(t *testing.T) {
	...
	// A command for some other switch must not move it.
	var other pb
