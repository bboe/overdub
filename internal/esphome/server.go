...
type Server struct {
	...
	// turn -- a wake gap, then an iptables pair that waits on netd, then the
	// tick's own reads. A guard reading it lets a peer resend the mode just
...
// What the poll last read, which is the only thing that can contradict the last
// apply. adbd can go down for reasons nothing here asked for, and without this
// the guard would turn away the very command that would bring it back: the
// select would sit on a position the device is not in and refuse to reassert
// it. A reading that agrees costs nothing.
