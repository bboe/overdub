...
presses swallowed, level unmoved. **Only the keys lift a mute.** A level set
from Home Assistant or a server lands and leaves the mute alone, because the
dump keeps the step a mute is holding -- it zeroes the percentage and not the
step -- so a set while muted counts from the right number and is simply
inaudible until something lifts it. Android's own key handling is what counts
from 0, which is the damage the grab exists to stop. A daemon that cannot take
the grab mutes anyway and says so. What it costs is Alexa's advanced factory
reset, mute and volume down together, which cannot complete while muted;
docs/hardware.md has the gesture.

**A node lost while the mute is still on is taken again, three times.** The
grab is the only thing standing between a press and a destroyed level, so
losing it silently is not an option -- but re-taking it on every loss is worse:
a node that fails every read spins the open, the ioctl and the close as fast as
the CPU allows, measured at 51,748 in 300 ms. Three tries, then the keys are
left alone and a line says so, and the count resets on the next clean unmute so
the following mute starts over. The keys are also never let go between an
unmute landing and the read back that says whether it took, because a press in
that window is the damage the grab exists to stop.
...
`MusicVolume` carries the flag for that reason. Nothing on this Dot has ever produced the mute it
