...
spanning 63 seconds cannot be on any caller's path. What is behind it now is the
switch rather than the API bind -- `disable` joins the advertising goroutine
before it closes the listener, so a synchronous ladder would hold switching
**off** for the whole minute. The API was the original reason and no longer is:
`enable` advertises on a goroutine of its own, so `Advertise` is not on the path
to the bind at all. It was, and with two sends a second apart the API bound two
seconds behind the Sendspin listener, measured on a Dot twice.
...
difference, so calling it with the list already in force is silent.
...
govern, paced only by the second between the two sends each of them makes.
